...
int main()
{
	...
	while(n--)
	{
		scanf("%s",in1);
		scanf("%s",in2);
		for(int a=0;a<10005;a++) for(int b=0;b<105;b++) dp[a][b] = 0;
		int len1 = strlen(in1);
		int len2 = strlen(in2);
		for(int a=0;a<len1;a++)
		{
			for(int b=0;b<len2;b++)
			{
				...
				if(b==0)
				{
					if(in1[a]==in2[b]) dp[a][b] += 1;
				}
				else
				{
					if(a!=0 && in1[a]==in2[b]) dp[a][b] = dp[a][b] + dp[a-1][b-1];
				}
			}
		}
		writeln(dp[len1-1][len2-1]);
	}
}
